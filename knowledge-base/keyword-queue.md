...
## 최우선 신규 Gap (Iteration 3 후 발굴, 5개)

1. **한국 강수 임계값 (지역·토양질감별)** — 현재 일반값(30-50mm) 만으로는 논토·밭의 토양질감 차이·지역 기후 반영 미흡. 한국 지역별 과습·가뭄 임계 mm 데이터 탐색(RISS, 농과원, 지역농기원)
2. **배 대기온 고온 피해 임계값** — FST 47.1°C는 과실표면온도. 재배자가 측정 가능한 대기온 기준값 필요. RDA 회색문헌/현장 자료 탐색
3. **감자/배 괴경/근부 냉해** — 지상부 냉해(잎)는 있으나 괴경·뿌리 냉해 온도 기준 미정. 냉저장 연구·포장 조사 자료 탐색
4. **토양질감별 과습 임계값** — 포식·식양·양토에서 30-50mm의 의미가 다름. 질감별 포장용수량·하한함수량 기반 환산 필요
5. **ETc 절대값 환산** — Mora 감자 수분수요는 ETc 상대값. 한국 기준 ETc 기준값(ET0) 확보 필요
