...
## Iteration 6 최우선 Gap (Iteration 5 후 발굴/이월, 2026-08-02)

> **사용자 지정 조건**: ①국내논문 우선 ②스캔본 지양 ③정확성 보장. Iteration 5에서 이 조건 그대로 사과 6개 갭을 병렬 조사한 결과는 아래 "Iteration 5 결과" 참고.

0. **[보류, 국내논문 0편] 사과 착색기(coloring) `temp_day`/`temp_night_min`** — Iteration 5에서 scout 실행함. 국내 문헌은 인용수 기준 통과작이 없었고 해외 5편(Blankenship 1987 등)만 확보돼 "국내 우선" 원칙상 이번엔 보류. 재개하려면: (a) 인용수 기준 완화(국내 학회 초록집 허용) 또는 (b) RDA 사과연구소 회색문헌으로 전환 — 사용자 확인 필요. 여전히 registry상 missing
1. **사과 EC national-scale 원문 확보** — TRKO202100009605(RDA 5차사업 보고서) 메타데이터는 확인됐으나 원문 다운로드가 JS세션에 막힘. **다음은 학술검색이 아니라 NTIS 재검색/RDA 국립농업과학원 직접 문의**가 필요(scout 재실행 불필요)
2. **사과 rainfall_by_stage 원문 확보** — 김미리·김승규(2014) 전 생육단계 강수계수표가 초록에 없어 KISS/DBpia 정식 원문 구매 필요
3. **사과 gdd Tbase** — 김수옥·윤진일(2010) 원문 재시도 1회, 실패 시 정식 심사 저널로 재검색("사과 발육모델 DVR", "사과 만개기 예측 적산온도")
4. **chilling_requirement 신규 지표 스키마 결정** (검색 아님, 팀 결정) — Iteration 5에서 Lee et al.(2015)로 filled 확보했으나 DB 스키마에 이 지표 자체가 없음
5. **사과 생육단계별(fruit_growth/maturity) 대기 낮기온** — Iteration 5에서 국내논문 0편으로 보류(#0과 동일 사유)
...
8. **한국 실측 강수 임계값 (지역·토양질감별)** — 배·오이·사과 전부 중국/외국 자료 (scout-4a, 이월)
9. **토양질감별 과습 임계값** / **ETc 절대값 환산** — 차순위 이월

> 관련 문서: `docs/guide-seed-known-issues.md`(P1/F-2), `docs/temperature-scoring.md`(§4), `knowledge-base/registry.md`(§6-1 Iteration 5 완료 요약)
...
- ~~사과 gdd Tbase 후보 검증~~ → 3편 전부 미해결(1 blocked, 2 reject) — 단 부산물로 **chilling_requirement 신규지표 발견**(위 #4)
- ~~사과 EC/rainfall 국내후보 검증~~ → 6편 확인했으나 전부 원문 미확보(caution/blocked) — 위 #1/#2로 이월
