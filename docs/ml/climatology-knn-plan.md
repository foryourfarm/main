...
>
> **현재 상태: 검증 완료(§2 통과), 본 구현 미착수.** 당장 진행하지 않고 보류 중.
...
견적 합계 **2~3시간**. DB는 이미 준비돼 있다(§6).

### 4-1. `backend/app/services/climatology_service.py` (핵심, ~40줄)

- [ ] 월별 평년치 값 객체 추가 — `WeatherClimatology` ORM 대신 담을 frozen dataclass.
...
- [ ] `load_climatology` 대체 분기 교체: `min(candidates)` → `sorted(...)[:k]` 후
      **월별·필드별** 가중평균. 가중치 `1/max(distance_km, 0.001)`, 그 필드를 가진 도너만
      대상으로 **재정규화**(도너 행 전체를 버리지 않는다).
- [ ] 어떤 필드의 모든 도너가 결측이면 그 필드는 `None` — 값을 지어내지 않는다.
- [ ] `0.0`을 결측으로 보지 않도록 `is None`으로만 판정(`_as_float` 주석의 기존 버그
      — 강원 산간 1월 평년기온 0.0℃ — 을 되살리지 말 것).
- [ ] 동거리 도너는 `region_id` 작은 쪽 — 결정론(CLAUDE.md §2).
- [ ] `ClimatologySource`에 `donors: tuple[tuple[str, float], ...] = ()` 추가.
      `substituted_from`/`distance_km`은 1순위 도너 값으로 **유지**(하위호환).
      frozen dataclass이므로 새 필드에 기본값 필수.
- [ ] `substitution_limitation()` 문구를 k개 기준으로 갱신 — §18-4(대체 사실 표기 의무).
      현재 `:192` "가장 가까운 {X}(약 {N}km)" → "가까운 X·Y·Z(약 N~M km) 평년치를 거리 가중 평균".
- [ ] 폴백(§18-5): 도너가 1개뿐이면 자동으로 현행과 동일한 동작이 된다(k=1). 도너가 0개면
...
- [ ] 타입힌트 3곳: `climatology_service.py:44`, `suitability_service.py:225`, `:394`.
...
- [ ] 1. 가중평균 손계산 일치 (도너 2개, 10km·20km → 가중 2:1)
- [ ] 2. 도너 하나가 특정 필드만 결측 → 그 필드만 재정규화, 다른 필드는 두 도너 다 사용
- [ ] 3. 모든 도너가 결측인 필드는 `None`이고 예외 없음
- [ ] 4. `0.0` 평년기온이 결측으로 떨어지지 않음 (회귀 테스트)
- [ ] 5. 도너가 1개뿐이면 현행과 동일 결과 (k=1 축퇴)
- [ ] 6. 같은 입력 2회 호출 결과 동일 (결정론)
- [ ] 7. `substitution_limitation()` 문구에 도너 지역명이 **전부** 포함

### 4-4. 문서

- [ ] FE 인계 문서 갱신(CLAUDE.md §3-7) — 응답에 `donors`가 추가되면 계약 문서 반영.
      대상 문서를 먼저 찾을 것(`docs/` 내 장기 적합도 계약 문서).
- [ ] 한계 표기 문구가 UI에 실제로 노출되는지 확인 — 텍스트만 바꾸고 화면 확인을 빠뜨리면 §18-4 위반.
