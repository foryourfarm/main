...
FE 관점에서 바뀌는 것:
- **같은 시/군의 밭이라도 읍면동이 다르면 점수가 다르게 나온다.** "같은 지역인데 왜 다르냐"는
  문의가 생길 수 있으니, 해당 밭의 `limitations` 문구를 반드시 노출한다.
- 보정된 기온은 `breakdown.temp_day.value`에 이미 반영돼 온다. FE가 따로 계산할 것 없다.
- 고도차가 20m 미만이면 보정하지 않고 문구도 안 뜬다(SRTM 표고 오차가 MAE 8.8m라
  그 언저리를 보정하면 잡음만 키운다).

한계(문구에 이미 들어 있다): 밭의 실측 고도가 아니라 **읍·면·동 대표점**의 고도다.
도시 법정동 3,152건은 그마저 없어 **시·군 대표점**으로 폴백한다.
