...
**적용 흐름 (`scripts/load_region_grid.py`):**
1. `docs/seed/region_grid_seed.csv`(기상청 배포 격자 엑셀 산출, 256행) 읽기
2. `region_grid(region_id, nx, ny)` 적재 — 256/256 매칭

> 위경도를 지오코딩해 `latlon_to_grid()`로 **계산하지 않는다**. 기상청 엑셀이 권위 있는
> 정답이고, 변환기는 격자 경계 41건에서 1칸씩 어긋난다(`scripts/gen_region_grid_seed.py`).
> `latlon_to_grid()`는 엑셀에 없는 좌표용 폴백이며, 역변환 `grid_to_latlon()`은 일조시간
> 계산에 필요한 지역 위도를 얻는 데 쓴다.
...
| `region_grid` | 격자좌표 매핑 (256) | ✅ `scripts/load_region_grid.py` |
| `kma_observation_point` | AWS 지점 + lat/lon | ⚠️ 테이블만(0015), 시드 0행 |
...
| 마이그레이션 | 목적 | 행 수 | 상태 |
|------------|------|------|------|
| ~~0014_region_grid_seed~~ | region_grid 시드 | — | ❌ 폐기(좌표 253개가 서울로 적재됨) |
| **0015_kma_observation_point** | 지점 테이블 생성만 | 0 | ✅ 스키마만(시드는 별도 PR) |
