...
**생육 기준 — PR #47(오이·상추·감자 문헌 갱신)에서 보류한 것**
- 감자 온도기준 모드 불일치: 문헌은 달력월(3~6월) 방식인데 `crop_growth_stage`는 파종후경과일
  방식 — 이번 PR은 유기물 기준만 포팅하고 온도기준은 보류(`[확인 필요]`).
- 상추 K/Ca/Mg 토양 기준: `soil_state`에 대응 컬럼이 없어 보류. 흙토람 API 클라이언트는
  이미 k/ca/mg를 파싱 중이라 스키마 컬럼 추가 + 스코어링 매핑만 남음.
...
- **장기 탭을 "앞으로 3개월"로 제한 (미착수, 2026-07-27 논의)**: 지금은 `compute_monthly_outlook`이
  올해 1~12월 전체를 계산해서 보여준다(`farms.py:133` 주석에 "연도 선택은 요구 생기면
  쿼리파라미터로"라고 이미 적혀 있던 자리). 실제로 신뢰할 수 있는 신호는 3개월전망뿐이라
  이 범위로 좁히는 게 핵심 — §12 원칙(평년치+3개월전망+작년실측)과도 맞는 방향.
  - 정공법: `build_monthly_rows`가 단일 `year`+`range(1,13)` 대신 (연도,월) 윈도우를 받게,
    `load_corrections`(`outlook_correction.py:61`, 단일 연도 가정)도 다년도 대응, 스키마
    `MonthlyOutlookEntry`(`schemas/suitability.py:44`)에 `year` 필드 추가(연도 경계 넘을 때
    셀 구분 안 됨 — 11·12월 시작 시 내년 1·2월 걸침). `test_monthly_outlook.py` 9개 케이스가
    "12개월·단일연도" 전제라 같이 손봐야 함. FE `LongTermPanel.tsx`는 `data.months` 그대로
    map이라 거의 안 건드려도 됨(상단 라벨만).
  - 더 싼 대안: 기존 12개월 계산은 그대로 두고 응답만 이번 달부터 3개 슬라이스. 스키마/테스트
    안 건드려도 되지만 11·12월에 시작하면 내년 1~2월이 그 12개월 배열에 없어서 3개월 중
    1~2개가 빠짐(연중 2달만 걸리는 구멍, `# ponytail:` 남기고 넘어가는 것도 선택지).
  - 진행 안 함(2026-07-27 결정) — 착수 전 방식(정공법 vs 슬라이스) 먼저 정할 것.
