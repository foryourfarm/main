...
### 결정 대기 — 밭 주소를 리 단위까지 받을 것인가 (권고: 받는다)
...
- **찬성 근거**: 정확도만이 아니라 **호출량도 준다** — 지금 리 5곳을 평균하느라
  13.6초를 쓰는데, 리를 알면 1회 3초다. 즉 현재 구현은 있는 정확도를 일부러 뭉개면서
  더 느리다. 리 목록은 `docs/seed/bjd_to_region.csv`에 15,209개가 이미 있고,
  스키마 변경도 없다 — `user_farm.bjd_code`가 법정동코드 10자리라 리 코드가 그대로 들어간다.
- **반대 근거**: 등록 단계가 하나 는다. 다만 시/군 datalist 검색을 넣어(#5) 체감 부담은 줄었고,
  귀농인은 자기 밭의 리를 안다("고창군 공음면 구암리"는 자연스러운 주소 단위).
- **필요한 일**: `district` 테이블 리 적재(5,066 → 약 20,000행), 등록 폼에 리 단계
  (동 지역은 리가 없으므로 조건부 노출), 기존 읍면동 밭은 지금의 리-풀 폴백을 그대로 유지.
- **미착수** — 등록 UX가 바뀌므로 착수 전 확인 필요.
...
**공음면 시설 EC 5.02 dS/m 값 검증 필요.** 시설재배는 염류집적으로 EC가 높게 나오는 게
정상 범위이긴 하나, 오이 허용치를 크게 넘어 **적용하면 farm 11 점수가 100 S에서 급락한다.**
지금까지 결측이라 안 보이던 게 드러나는 것이므로 맞는 값이면 그게 정직한 결과다 —
다만 급락을 버그로 오인하지 않도록 미리 적어 둔다.
