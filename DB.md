...
game_save 1──N cultivation_log
```

- `region`, `crop`, `crop_growth_guide`, `soil_data`, `weather_history`, `region_grid` = 마스터/사전적재.
...
| season_weeks | INT | NOT NULL | 시즌 총 주차 수(미정, 시드로) |
...
- 유니크: users.email, farm_plot(save,region,crop), soil(region), weather(region,week), suit(region,crop,week).
- 조회 인덱스: game_save(user), weather(region,week), log(save,week).
- CHECK: score 0..100, grade 화이트리스트, week ≥1.
...
- 작물별 시즌 총 주차 수(`crop.season_weeks`).
